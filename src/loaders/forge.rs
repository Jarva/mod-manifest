...
pub fn from_cf(url: String, files: Vec<File>) -> Result<String> {
  let file_version_finder = Regex::new(r"(?:^|[^0-9])(?<version>\d+(?:\.\d+)*)(?=[^0-9]|$)").unwrap();
  ...
  for file in files {
    ...
    for capture in captures {
      if capture.is_ok() {
        let found = capture.unwrap().name("version").unwrap().as_str().to_string();
        let did_find = file.game_versions.iter().any(|version| found.contains(version.as_str()));
        if !did_find {
    ...
    for version in versions {
      ...
      mcversion.insert(file_version.clone().unwrap().to_string(), format!("{}/files/{}", url, file.id));
      if !promos.contains_key(version) {
        promos.insert(version.to_string(), file_version.clone().unwrap().to_string());
      }
    }
  }
